...
import type { IProps } from "../../Editor";
// import './index.less';
...
const DragBar: React.FC<IDragBarProps> = (props) => {
  ...
  function handleMouseMove(event: Event) {
    if (dragRef.current) {
      ...
      const newHeight =
        dragRef.current.height + clientY! - dragRef.current.dragY;
      if (newHeight >= props.minHeight && newHeight <= props.maxHeight) {
        onChange &&
          onChange(dragRef.current.height + (clientY! - dragRef.current.dragY));
      }
    }
  }
  ...
  function handleMouseDown(event: Event) {
    ...
    $dom.current?.addEventListener("touchmove", handleMouseMove, {
      passive: false,
    });
    $dom.current?.addEventListener("touchend", handleMouseUp, {
      passive: false,
    });
  }
  ...
  useEffect(() => {
    if (document) {
      $dom.current?.addEventListener("touchstart", handleMouseDown, {
        passive: false,
      });
